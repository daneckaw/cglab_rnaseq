...
```{r cbs_f_deseq, fig.height=6, fig.width=8, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r cbs_v_deseq, fig.height=6, fig.width=8, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r cbs_v_deseq_excludebadc, fig.height=4, fig.width=5, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r cbs_vf_deseq, fig.height=6, fig.width=8, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r cbs_vf_deseq_excludebadc, fig.height=4, fig.width=5, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r bg2_f_deseq, fig.height=6, fig.width=8, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r bg2_v_deseq, fig.height=6, fig.width=8, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r bg2_fv_deseq, fig.height=6, fig.width=8, eval=TRUE, message=FALSE, warning=FALSE}
...
sprintf("Upregulated CBS138 FCZ vs CT: %d", nrow(DEGdf_up2x_FDR5_cf))
# sprintf("Printing the first 10:")
# print(DEGdf_up2x_FDR5_cf, n = 10)


sprintf("Downregulated CBS138 FCZ vs CT: %d", nrow(DEGdf_down2x_FDR5_cf))
...
sprintf("Upregulated in CBS138 VCZ vs CT: %d", nrow(DEGdf_up2x_FDR5_cv))
# sprintf("Printing the first 10:")
# print(DEGdf_up2x_FDR5_cv, n = 10)


sprintf("Downregulated in CBS138 VCZ vs CT: %d", nrow(DEGdf_down2x_FDR5_cv))
# print("Printing the first 10:")
...
sprintf("Upregulated in BG2 FCZ vs CT: %d", nrow(DEGdf_up2x_FDR5_bf))
# sprintf("Printing the first 10:")
# print(DEGdf_up2x_FDR5_bf, n = 10)


sprintf("Downregulated in BG2 FCZ vs CT: %d", nrow(DEGdf_down2x_FDR5_bf))
# print("Printing the first 10:")
...
sprintf("Upregulated in BG2 VCZ vs CT: %d", nrow(DEGdf_up2x_FDR5_bv))
# sprintf("Printing the first 10:")
# print(DEGdf_up2x_FDR5_bv, n = 10)


sprintf("Downregulated in BG2 VCZ vs CT: %d", nrow(DEGdf_down2x_FDR5_bv))
# print("Printing the first 10:")
...
Using curated orthologs between C.glabrata CBS138 and S.cerevisiae (downloaded from candidagenome.org), I identified standard and systematic _S. cerevisiae_ names for _C.glabrata_ genes up- and down-regulated in FCZ vs CT.
...
```{r c_b_d, fig.height=6, fig.width=8, eval=TRUE,  message=FALSE, warning=FALSE}
...
```{r c_b_v, fig.height=6, fig.width=8, eval=TRUE,  message=FALSE, warning=FALSE}
...
```{r c_b_f, fig.height=6, fig.width=8, eval=TRUE, message=FALSE, warning=FALSE}
...
sprintf("CT upregulated: %d, downregulated: %d", nrow(DEGdf_up2x_FDR5_cbd), nrow(DEGdf_down2x_FDR5_cbd))
...
a) Principal Component Analysis shows that RNA-seq samples cluster by strain and by azole drug treatment. Principal components 1 and 2 of the regularized logarithm of counts per gene are shown for all samples; see methods for details.
b) Between-sample variance is concentrated in principal components 1 and 2, that panel a shows cluster by strain and azole treatment.
c) There are extensive baseline gene expression differences between strains BG2 and CBS138.
d) A consistent azole-dependent transcriptomic response is identified by pooling azole-dependent differential expression across both FCZ and VCZ drugs across both strains BG2 and CBS138.
e) There are minimal strain-dependent differences in drug-induced gene expression, detected using the interaction term in a DESeq2 analysis with both factors (`design = ~ Strain * Drug`).
See methods for details of differential gene expression analysis across 3 biological replicates using DESeq2, and supplementary figure SX for additional pairwise differential expression plots.

```{r summary_plot, fig.height=6, fig.width = 6.85, echo=FALSE, message=FALSE, warning=FALSE}
guide_legend_pc12 <-
  guide_legend("Strain, Treatment",
               position = "right",
               ncol = 1)
...
p_summary <- 
  plot_grid(p_summary_toprow, 
            p_summary_volcanos, 
            ncol = 1)
p_summary


ggsave("../figures/RNAseqsummary_nodesign.png",
       p_summary, 
       height=6, width = 6.85, 
       dpi = 350,
       bg = 'white')
...
## Figure 4 - Genes in known pathways are specifically responsive to azole drugs.
...
```{r deseq_4x_create, echo = FALSE}
# make data frame 
deseq_df_cboth <- bind_rows(
  deseq_df_cf %>% 
    mutate(Strain = "CBS138",
           Drug   = "FCZ"),
  deseq_df_cv %>% 
    mutate(Strain = "CBS138",
           Drug   = "VCZ")
) %>%
  rename(gene = "gene_id_cbs138") %>%
  left_join(rbbh, by = "gene_id_cbs138")

deseq_df_bboth <- bind_rows(
  deseq_df_bf %>% 
    mutate(Strain = "BG2",
           Drug   = "FCZ"),
  deseq_df_bv %>% 
  ...
  rename(gene = "gene_id_bg2") %>%
  left_join(rbbh, by = "gene_id_bg2")

deseq_df_4x <- 
  bind_rows(deseq_df_cboth, deseq_df_bboth) %>%
  select(gene_id_cbs138, gene_id_bg2, Strain, Drug,
         baseMean, log2FC, stderror, padj) %>%
  mutate(Strain_Drug = paste(Strain, Drug, sep = "_") %>%
           as_factor)
deseq_df_4x
```
...
Make a heatmap of DGE in all 4 conditions using facet_grid.

```{r genelist_heatmap, fig.height = 8, fig.width = 3.4}
...
This heatmap wwas then edited manually using inkscape, in `figures/genelist_heatmap_manualedit.svg`.


```{r genelist_heatmap_alternativeapproach, fig.height = 6, fig.width = 3, eval = FALSE, echo = FALSE}
# alternative approach to genelist heatmap with facet_wrap
# abandoned as could not get it to display titles properly
# or otherwise not heights properly

genecategoriesA <- c("azole specific", "virulence factor", "cell wall")

genelist_heatmapA <- 
  ggplot(data = filter(deseq_4x_genelist, Group %in% genecategoriesA)) +
  genelist_heatmap_pieces

genecategoriesB <- c("CRZ1-regulated", "stress response")

genelist_heatmapB <- 
  ggplot(data = filter(deseq_4x_genelist, Group %in% genecategoriesB)) +
  genelist_heatmap_pieces
...
genelist_heatmap_alt <- 
  plot_grid(genelist_heatmapA +
            ...
            genelist_heatmapB + 
              theme(plot.margin = 
                      unit(c(18, 6, 68, 6), 
                                "points")) + 
              guides(fill = 
                guide_colourbar("log2\nfold-\nchange",
                             position = "bottom",
                             direction = "vertical",
                             theme(legend.title.position = "left",
                                   legend.justification.bottom = c(0,0.5)))),
            ncol = 2,
            rel_widths = c(1, 1.2),
            labels = c("a","b"))
...
# ggsave("../figures/genelist_heatmap.png",
#        genelist_heatmap, 
#        height = 6, width = 4.2, 
#        dpi = 350,
#        bg = 'white')

# genelist_dotmap - commenting out for now.
# genelist_dotmap <- 
