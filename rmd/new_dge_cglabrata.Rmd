...
head(counts_1) # CBS138, raw
head(counts_2) # BG2, raw
```
...
```{python pairplot1, fig.height = 4, fig.width=10, warning=FALSE, message=FALSE, echo=FALSE}
...
Here I look at ovelap between genes upregulated/downregulated in FLUC treatment between CBS138 and BG2.
This includes:
...
counts_all_2_rbbh = merge(py$counts_all_2, rbbh, by.x="Geneid", by.y="gene_id_bg2")
...
counts_all_joined = merge(py$counts_all_1, counts_all_2_rbbh,by="Geneid")
...
counts_only = subset(counts_all, select=-c(Geneid))
print(head(counts_only))

colnames(counts_only) = NULL # DESeqDataSetFromMatrix throws a col name error unless I do this, even though I'm pretty sure that column names are correct. 
...
pc12 = ggplot(data = pcdf_rlog,
       aes(colour = Condition, shape = factor(Temp))
       ) +
...
The outlier: CBS138_RPMI_VORI (light purple) clusters with other bio reps in PC1-PC2, but not in PC3.
